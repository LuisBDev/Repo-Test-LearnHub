...
const SingleCourse = () => {
  ...
  const [updateStateQuestion, setUpdateStateQuestion] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadCompletedQuestions = async () => {
      const { data } = await axios.post(`/api/list-completed-question`, {
        courseId: course._id,
      });
      console.log("PREGUNTAS COMPLETADAS : ", data);
      console.log("Cantidad de preguntas completadas: ", data.length);
      console.log("Cantidad total de preguntas: ", course.questions.length);
  ...
  const markCompletedQuestion = async () => {
    try {
      const { data } = await axios.post(`/api/mark-completed-question`, {
        courseId: course._id,
        questionId: currentQuestion,
      });
      console.log(data);
      setCompletedQuestions([...completedQuestions, currentQuestion]);
      setUpdateStateQuestion(true); // Agregar esta línea
      console.log("PREGUNTAS COMPLETADAS : ", completedQuestions);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const renderQuestionBarItems = () => {
    return course.questions.map((question, index) => {
      ...
      return (
        <Item
          onClick={() => !isCompleted && handleQuestionClick(index)}
          key={question.id}
          icon={<Avatar>{index + 1}</Avatar>}
        >
          ...
          {isCompleted ? (
            <CheckCircleOutlined
              className="float-right text-primary mt-3 ml-2"
              style={{ marginTop: '13px' }}
            />
          ) : (
            <MinusCircleFilled
              className="float-right text-danger mt-3 ml-2"
              style={{ marginTop: '13px' }}
            />
          )}
        </Item>
